...
void
proc_freekernalpagetable(pagetable_t pagetable, uint64 kstack, uint64 sz)
{
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    // if(uvmcopy2(p->pagetable, p->kernal_pagetable, sz, n) != 0) {
...
fork(void)
{
  ...
  for (j = 0; j < p->sz; j+=PGSIZE){
    ...
    kernelPte = walk(np->kernal_pagetable, j, 1);
    *kernelPte = (*pte) & ~PTE_U;
  }
  np->sz = p->sz;
