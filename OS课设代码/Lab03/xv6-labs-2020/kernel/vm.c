...
kvmpa(uint64 va)
{
  ...
  pte = walk(myproc()->kernal_pagetable, va, 0);
  if(pte == 0)
    panic("kvmpa");
  if((*pte & PTE_V) == 0)
    panic("kvmpa");
...
int
uvmcopy2(pagetable_t upagetable, pagetable_t kpagetable, uint64 start, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = PGROUNDUP(start); i < start + sz; i += PGSIZE){
    if((pte = walk(upagetable, i, 0)) == 0)
      panic("copypgtable_u2k: pte should exist");
    if((*pte & PTE_V) == 0)
      panic("copypgtable_u2k: page not present");
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte) & ~PTE_U;// 将PTE_U清零，因为内核不能访问PTE_U被设置的页！！！！
